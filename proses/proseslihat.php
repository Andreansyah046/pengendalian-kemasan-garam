...
switch ($op){
    ...
    if ($execute->num_rows > 0){
        ...
        while($data=$execute->fetch_array(MYSQLI_ASSOC)){
            ...
            <tr id='data'>
                ...
                <a class=\"btn btn-yellow\" data-a=\"nilai $data[nilai] dalam $data[namaKriteria]\" id='hapus' href='../?page=hapus&op=subkriteria&id=".$data['id_nilaikriteria']."'><i class='fa fa-trash-alt'</a></td></div>
            </tr>";
            $no++;
        }
    }else{
    ...
    case 'nilai':
        if (!empty($id)) {
            $where="WHERE nilai_supplier.id_produk ='$id'";
        }else{
            $where=null;
        }
            ...
            while($data=$execute->fetch_array(MYSQLI_ASSOC)){
               echo"
                <tr id='data'>
                    ...
                    <a class=\"btn btn-yellow\" data-a=\".$data[namaBarang] - $data[namaSupplier]\" id='hapus' href='../?page=hapus&op=nilai&id=".$data['id_supplier']."'><i class='fa fa-trash-alt'></i></a></td>
                </div></tr>";
                $no++;
            }
        }else{
